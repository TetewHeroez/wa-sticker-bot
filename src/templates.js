module.exports = {
  INTRO: {
    name: "sticker_intro_v1",
    language: "id",
  },
  HELP_MESSAGE: `*Konnichiwa, Sensei...* 😊👋

_Cara Pakai:_
📸 Kirim *gambar* → Sticker statis
🎥 Kirim *video* (maks 20 detik) → Sticker animasi
📎 Kirim *dokumen gambar/video* (PNG, JPG, MP4, dll) → Juga dikonversi ke sticker!
💬 Kirim *pesan teks* → Dijawab oleh AI

_Brat Sticker:_
🟢 Ketik *.brat [teks]* → Brat sticker statis
🟢 Ketik *.bratgif [teks]* → Brat sticker animasi (per kata)

_Perintah:_
- Ketik *stats* untuk melihat statistik bot
- Ketik *reset* untuk reset percakapan AI
- Ketik *help* untuk melihat menu ini

_Tips:_
- Kirim gambar lewat galeri *atau* sebagai dokumen/file, keduanya bisa!
- Gunakan gambar/video dengan rasio kotak (1:1) untuk hasil terbaik
- Video akan dipotong otomatis jika lebih dari 20 detik
- Ukuran file maksimal 16 MB
- AI mengingat konteks percakapan selama 30 menit`,
};
